-- Script final pour appliquer la migration CategorieAgent
-- Exécuter ce script directement dans votre client MySQL

-- Marquer les migrations précédentes comme appliquées si nécessaire
...
-- Appliquer les changements CategorieAgent
...
-- Vérification
SELECT 'Migration CategorieAgent appliquée avec succès!' as Result;
SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` LIKE '%CategorieAgent%' ORDER BY `MigrationId`;
